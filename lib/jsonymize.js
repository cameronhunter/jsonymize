...
Jsonymize.fn.anonymize = function(stream) {
  var anonymizeActions = this._fields.reduce(function(obj, alias) {
    ...
    var possibilities = [generatorOverride, alias, field].filter(notNull);
    var selected = findGenerator(possibilities, this.chance);
    if (selected) {
...
function handle(chance) {
  return function(value, path) {
    if (!path.length) return;

    var selected = findGenerator([type(value)].filter(notNull), chance);
