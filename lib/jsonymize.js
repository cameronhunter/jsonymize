...
export default class Jsonymize {
  constructor(options = {}, chance = new Chance()) {
  ...
  anonymize(stream) {
    ...
    const actions = fields.reduce((action, alias) => {
      ...
      return Object.assign({}, action, {
        [field]: (value, path) => {
          const generators = findGenerators(chance, generatorOverride, alias, field, ...type(value));
...
function type(value) {
  ...
  ].map(([type, predicate]) => (predicate && predicate(value) ? type : undefined));
}
